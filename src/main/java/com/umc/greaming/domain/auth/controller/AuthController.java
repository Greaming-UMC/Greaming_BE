...
public class AuthController {
     ...
     * @param response 새로운 리프레시 토큰을 쿠키로 설정하기 위한 응답 객체
     * @return 새로운 액세스 토큰
    ...
    public ResponseEntity<ApiResponse<TokenResponse>> reissueToken(
        ...
        // 새로운 리프레시 토큰을 쿠키로 설정
        cookieConfig.addRefreshTokenCookie(response, request, reissueResponse.refreshToken());
    ...
    private String extractRefreshTokenFromCookie(HttpServletRequest request) {
        ...
        return Arrays.stream(cookies)
                .filter(cookie -> CookieConfig.REFRESH_TOKEN_COOKIE_NAME.equals(cookie.getName()))
                .map(Cookie::getValue)
                .findFirst()
                .orElse(null);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> logout(
        ...
        // 리프레시 토큰 쿠키 삭제
        cookieConfig.deleteRefreshTokenCookie(response, request);
